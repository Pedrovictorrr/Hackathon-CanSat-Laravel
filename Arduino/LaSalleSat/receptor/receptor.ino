...
void listening_accelerometer() {
  ...
  if (next_type == 1) {
    ...
    Serial.print("|  Acel:");
    Serial.print("  X:");
    Serial.print(data.acelX);
    Serial.print(" Y:");
    Serial.print(data.acelY);
    Serial.print(" Z:");
    Serial.println(data.acelZ);

    Serial.print("|  Giro:");
    Serial.print("  X:");
    Serial.print(data.giroX);
    Serial.print(" Y:");
    Serial.print(data.giroY);
    Serial.print(" Z:");
    Serial.println(data.giroZ);
    Serial.println("|");
...
void listening_MQ135() {
  ...
  if (next_type == 0) {
    ...
    Serial.println(data.CO2 + 400);
    Serial.print("|  Álcool: ");
    Serial.println(data.Alcohol);
    Serial.print("|  Amónio: ");
...
// Transformando em json e salvando em arquivo //
void json() {
    BMP_package data;
  radio.read(&data, sizeof(data));

  
  exporter.add("Temperatura", data.Temperatura);
  exporter.add("Altitude", data.Altitude);
  exporter.add("Pressao", data.Pressao);

  MQ135_package MQ;
  radio.read(&MQ, sizeof(MQ));

  exporter.add("CO", MQ.CO);
  exporter.add("CO2", MQ.CO2);
  exporter.add("Alcohol", MQ.Alcohol);
  exporter.add("NH4", MQ.NH4);
  exporter.add("Acetona", MQ.Acetona);
  
  Accelerometer_package ACC;
  radio.read(&ACC, sizeof(ACC));

  exporter.add("AcelX", ACC.acelX);
  exporter.add("AcelY", ACC.acelY);
  exporter.add("AcelZ", ACC.acelZ);
  exporter.add("GiroX", ACC.giroX);
  exporter.add("GiroY", ACC.giroY);
  exporter.add("GiroZ", ACC.giroZ);
  exporter.exportJSON();
}
